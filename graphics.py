...
def load_graphics():
    ...
    # Had to make this change, because the alternative was atrocious , by Sam
    # The original code goes from 1 to 25, so that's why the plus 1
    for i in range(25):
        loaded_dict[f"big_hp_{i + 1}"] = hp_big_sheet[i]
    ...
    for i in range(101):
        loaded_dict[f"hp_{i}"] = hp_sheet[i]
        loaded_dict[f"en_{i}"] = en_sheet[i]
        loaded_dict[f"xp_{i}"] = xp_sheet[i]
